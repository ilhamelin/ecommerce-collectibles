...
export default function OrderTrackingPage() {
  ...
  const [afterShipResult, setAfterShipResult] = useState<TrackingResult | null>(null);
  ...
  // Fetch order from DB and live AfterShip tracking
      ...
      fetch(`/api/tracking/${orderIdParam}`)
        ...
        .then((trackData) => {
          if (trackData.success && trackData.data) {
            setAfterShipResult(trackData.data);
            if (trackData.data.status === "DELIVERED") {
      ...
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <div className="p-6 rounded-3xl bg-white border border-[#E5E5E5] shadow-sm flex flex-col md:flex-row items-start md:items-center justify-between gap-6">
            ...
            <div className="flex flex-wrap items-center gap-2.5">
              ...
              {afterShipResult && (
                <span className="px-2.5 py-0.5 rounded-full bg-slate-100 text-slate-700 text-[11px] font-bold border border-slate-200 flex items-center gap-1">
                  ...
                  {afterShipResult.source === "AFTERSHIP_LIVE"
                    ? "AfterShip Live"
                    : afterShipResult.source === "AFTERSHIP_SANDBOX"
                    ? "AfterShip Sandbox"
                    : "AfterShip Adapter"}
                </span>
              )}
          ...
          <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3 w-full md:w-auto">
            {afterShipResult?.externalTrackingUrl && (
              <a
                href={afterShipResult.externalTrackingUrl}
                target="_blank"
                rel="noopener noreferrer"
                className="px-4 py-2.5 rounded-xl bg-orange-600 hover:bg-orange-700 text-white font-bold text-xs transition flex items-center justify-center gap-2 shadow-xs cursor-pointer"
              >
                <span>Ver en AfterShip</span>
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <div className="p-6 rounded-3xl bg-white border border-[#E5E5E5] shadow-sm space-y-5">
              ...
              <div className="space-y-6 relative before:absolute before:inset-0 before:left-3.5 before:w-0.5 before:bg-[#E5E5E5]">
                {afterShipResult && afterShipResult.checkpoints && afterShipResult.checkpoints.length > 0 ? (
                  // Checkpoints sincronizados vía AfterShip / API Courier
                  afterShipResult.checkpoints.map((cp, idx) => {
                    const isLatest = idx === afterShipResult.checkpoints.length - 1;
                    const isDeliveredCp = cp.status === "DELIVERED";
                    const isOutForDelivery = cp.status === "OUT_FOR_DELIVERY";
                    const cpDate = new Date(cp.timestamp);
                    const formattedCpTime = {
                      date: cpDate.toLocaleDateString("es-CL", { day: "2-digit", month: "short", year: "numeric" }),
                      time: cpDate.toLocaleTimeString("es-CL", { hour: "2-digit", minute: "2-digit" }),
                    };
                        ...
                        <div className="flex-1 bg-white p-4 rounded-2xl border border-[#E5E5E5] shadow-2xs">
                          <div className="flex flex-wrap items-center justify-between gap-2">
                            ...
                            <span className="text-xs font-mono text-[#666666]">
                              {formattedCpTime.date} • {formattedCpTime.time} hrs
                            </span>
                          </div>
                          <p className="text-xs text-[#666666] mt-1">
                            Ubicación: <strong className="text-[#1A1A1A]">{cp.location}</strong>
                          </p>
                          {cp.rawTag && (
                            <span className="inline-block mt-2 px-2 py-0.5 rounded text-[10px] font-mono bg-slate-100 text-slate-700 font-bold border border-slate-200">
                              Tag: {cp.rawTag}
                            </span>
                          )}
                        </div>
                  ...
                  // Fallback de bitácora estimada chilena estándar
                  <>
                    {/* Event 4: Entregado */}
