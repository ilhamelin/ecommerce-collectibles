...
interface ProductDetailClientProps {
  initialProduct?: ProductDomainEntity | null;
  slug?: string;
}

export default function ProductDetailClient({ initialProduct, slug: propSlug }: ProductDetailClientProps) {
  const params = useParams();
  const rawSlug = propSlug || (params?.slug as string) || "";
  const slug = rawSlug.toLowerCase();

  const [product, setProduct] = useState<ProductDomainEntity | null>(initialProduct || null);
  ...
  useEffect(() => {
    ...
    catalogClient.getCatalog()
      .then(async (allProducts) => {
        ...
        // Find target product from cache or direct query if not already passed via SSR
        if (!product) {
        ...
        <div className="md:hidden fixed bottom-0 left-0 right-0 z-40 bg-[#0F172A]/95 backdrop-blur-md border-t border-white/10 p-3 shadow-2xl flex items-center justify-between gap-3">
          <div className="flex items-center gap-2.5 min-w-0">
            ...
            <div className="min-w-0">
              <span className="text-[11px] font-bold text-white line-clamp-1 block leading-tight">
                {product.name}
              </span>
              <div className="flex items-center gap-1.5 mt-0.5">
                {isPreOrder && selectedDepositMode === "PARTIAL" ? (
                  <>
                    <span className="text-xs font-black text-[#FF6B35]">
                      {formatCLP(depositAmount)}
                    </span>
                    <span className="text-[9px] px-1.5 py-0.2 rounded bg-[#FF6B35]/20 text-[#FF6B35] font-extrabold uppercase">
                      20% Reserva
                    </span>
                  </>
                ) : (
          ...
          <button
            type="button"
            disabled={product.stockAvailable <= 0 && !isPreOrder}
            onClick={handleAddToCart}
            className={`px-4 py-2.5 rounded-xl font-black text-xs transition flex items-center justify-center gap-1.5 shrink-0 shadow-lg ${
              justAdded
                ? "bg-emerald-500 text-white"
                : product.stockAvailable <= 0 && !isPreOrder
                ? "bg-gray-700 text-gray-400 cursor-not-allowed"
                : "bg-[#FF6B35] hover:bg-[#E85D2A] text-white active:scale-95"
            }`}
          >
            {justAdded ? (
              <>
